...
| Phase | Scope | Deliverables | Regression gate | Recommended model / effort | Status |
|------|-------|--------------|-----------------|----------------------------|--------|
| **0 — Harness & scaffolding** | Test infra + `CLAUDE.md`, no parser logic change | `CLAUDE.md`; `tsconfig.test.json`; `test/run.ts`; all `test/fixtures/*.vhd` with `-- EXPECT` headers; `package.json` `test` script + `tsx` devDep | `npm test` runs; `single_process.vhd` green; corner-case fixtures may be red (expected-fail, documented) | **Sonnet 4.6, low–medium** (mechanical) | Done — [`a78df98`](https://github.com/Bastocus/vhdl-fsm-diagram/commit/a78df98) |
